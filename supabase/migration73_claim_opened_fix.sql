-- ============================================================
-- Migration 73: claim_opened() failed on a plain ?claim visit.
-- (Applied automatically by the build; nothing to paste.)
--
-- With no space named, the function read a field of a record it had
-- never filled, which PL/pgSQL treats as an error, so the visit was
-- neither logged nor pinged. Plain variables instead of a record.
-- ============================================================
...
begin
  ...
  what := case when v_name is not null
               then v_name || coalesce(' in ' || nullif(v_city, ''), '')
