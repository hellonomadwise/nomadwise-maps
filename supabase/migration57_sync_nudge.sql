-- ============================================================
-- Migration 57: the database asks GitHub to run the website push
-- the moment there is work for it.
-- (Applied automatically by the build; nothing to paste.)
--
-- Why: the "every ten minutes" schedule on GitHub is best-effort and
-- in practice runs every few hours. Queueing and approving in the app
-- should not wait for that. So whenever a venue is queued, approved
-- or edited while queued, the database calls GitHub's API and starts
-- the "Website push (queued spaces)" workflow itself, at most once
-- every two minutes. The schedule stays as a backstop.
--
-- One secret is needed, kept in Supabase Vault (never in this repo):
--   name   github_actions_token
--   value  a fine-grained GitHub token for hellonomadwise/nomadwise-maps
--          with Actions: Read and write
-- Until it exists the trigger does nothing and nothing breaks.
-- ============================================================

create extension if not exists pg_net;

create table if not exists public.sync_nudges (
  id int primary key,
  last_at timestamptz,
  last_status int
);
insert into public.sync_nudges (id) values (1) on conflict do nothing;
alter table public.sync_nudges enable row level security;
...
drop trigger if exists trg_nudge_website_sync on public.venues;
create trigger trg_nudge_website_sync
  after insert or update on public.venues
  for each row execute function public.nudge_website_sync();
